async function initTallerSecarropas() {
    await loadTallerSecarropasData();
}

async function loadTallerSecarropasData() {
    try {
        const res = await api('/servicios/taller/secarropas');
        ...
        createAccessDatasheet(res, 'taller-secarropas-grid', columns);
    } catch (e) {
        console.error('Error loading taller-secarropas data:', e);
    }
}
