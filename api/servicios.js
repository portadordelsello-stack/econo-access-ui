...
router.get('/taller-cristian', (req, res) => {
    const query = getBusinessQuery(`s.ingreso_taller = 1 AND s.entregado = 0 AND s.pasa_a_stock = 0 AND s.tecnico = 'Cristian'`);
    res.json(db.prepare(query).all());
});

router.get('/taller-joel', (req, res) => {
    const query = getBusinessQuery(`s.tecnico = 'Joel' AND s.ingreso_taller = 1 AND s.entregado = 0 AND s.pasa_a_stock = 0 AND s.terminado = 0`);
    res.json(db.prepare(query).all());
});
...
router.get('/recaudacion-futura-total', (req, res) => {
    const targetDate = req.query.date || tomorrowDate();
    const row = db.prepare(`
        SELECT COALESCE(SUM(s.presupuesto), 0) as SumaDePresupuesto
        FROM servicio s
...
router.get('/producto-presente-mes', (req, res) => {
    const targetMonth = req.query.month || '2022-03';
    const data = db.prepare(`
        SELECT s.id_servicio, s.presupuesto, s.acepta, s.cita_entrega, s.factura, s.entregado, s.aparato, s.marca_modelo, s.servicios_convenidos,
               c.calle, c.numero_direccion, c.depto, c.piso
        FROM servicio s
        JOIN clientes c ON s.id_cliente = c.id_cliente
        WHERE strftime('%Y-%m', parse_access_date(s.cita_entrega)) = ?
